...
def seed_default_users(apps, _schema_editor):
    User = apps.get_model('auth', 'User')
    Profile = apps.get_model('issues', 'Profile')
...
def unseed_default_users(apps, _schema_editor):
    User = apps.get_model('auth', 'User')
    User.objects.filter(username__in=[username for username, _ in DEFAULT_USERS]).delete()


class Migration(migrations.Migration):

    dependencies = [
        ('issues', '0025_profile_api_key'),
    ]

    operations = [
        migrations.RunPython(seed_default_users, reverse_code=unseed_default_users),
    ]